module DB
    ...
    def user_exists(name)
        taken_name = db().execute("SELECT * FROM users WHERE name IS ?", name)
        taken_name != []
    end
    ...
    def get_author_of_post(post_id)
        db().execute("SELECT name FROM users WHERE id IN (SELECT author_id FROM posts WHERE id=?)", post_id)[0][0]
    end

    def add_like(user, post)
        db().execute("INSERT INTO user_likes_post (user_id, post_id) VALUES (?, ?)", [user, post])
    end

    def remove_like(user, post)
        db().execute("DELETE FROM user_likes_post WHERE user_id=? AND post_id=?", [user, post])
    end

    def password_is_correct(username, input_password)
        real_password = db().execute("SELECT password FROM users WHERE name=?", username)
    ...
    def get_user_id(name)
        db().execute("SELECT id FROM users WHERE name=?", name)[0][0]
    end
